...
想看到 simulation、waveform、Analysis 頁面完整結果、AI review 或 Compare 結果時，請同時上傳 design file 與對應 testbench，例如：
...
| `11_bad_style_mix/`      | 混合風格 / lint 測試 | `bad_style_mix.v`                                 | unused wire、混合時序/組合寫法、AI review                     |
...
- LogViewer 與 AI review 可用來解讀失敗原因
...
如果系統能在 Analysis 頁顯示錯誤、LogViewer 顯示 `$fatal`，並在 AI review 中說明可能原因，就代表錯誤偵測流程有正常運作。
...
- 如果看不懂結果，先檢查是否有 error、warning、waveform 與 AI review。
