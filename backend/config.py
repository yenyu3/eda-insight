"""
config.py — 集中管理所有環境變數與常數

由 app.py 在 load_dotenv() 之後 import，其餘模組再從這裡讀取設定。
"""

import os
from pathlib import Path

BASE_DIR = Path(__file__).parent

# Flask
FLASK_DEBUG: bool = os.environ.get("FLASK_DEBUG", "0") == "1"
FLASK_PORT: int = int(os.environ.get("FLASK_PORT", "5050"))
CORS_ORIGINS: list[str] = ["http://localhost:5173"]

# Storage
UPLOAD_DIR: str = str(BASE_DIR / "uploads" / "runs")
DB_PATH: str = str(BASE_DIR / "eda_platform.db")

# AI Provider
AI_PROVIDER: str = os.environ.get("AI_PROVIDER", "anthropic").lower()
ANTHROPIC_MODEL: str = os.environ.get("ANTHROPIC_MODEL", "claude-sonnet-4-20250514")
GEMINI_MODEL: str = os.environ.get("GEMINI_MODEL", "gemini-2.5-flash")
MAX_TOKENS: int = 1024

# Pipeline
FIXED_PIPELINE: list[str] = ["lint", "simulate", "synthesize", "dependency"]
VALID_STEPS: set[str] = {"lint", "simulate", "synthesize", "dependency"}
USE_FIXED_PIPELINE: bool = os.environ.get("USE_FIXED_PIPELINE", "true").lower() == "true"
